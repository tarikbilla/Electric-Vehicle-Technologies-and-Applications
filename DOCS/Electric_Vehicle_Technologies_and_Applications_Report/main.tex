...
% Reference entries contain long unhyphenatable titles and DOIs; allow
% looser word spacing inside the bibliography rather than overfull lines.
\AtBeginBibliography{\sloppy\hbadness=10000\relax}
...
% Headings are justified by default, so a title too long for one column line
% is stretched across the first line and reported as an underfull \hbox. In a
% 258pt column several headings wrap. These are the article-class definitions
% with \raggedright added and the spacing left untouched; ragged-right headings
% are in any case the normal setting for a narrow two-column measure.
\newenvironment{loosepar}{\par\sloppy\hbadness=10000\relax}{\par}
...
Values are drawn from the manufacturer's own documentation where it
exists~\cite{tesla2024}, from the procedures that type approval
prescribes~\cite{gtr15,epacoastdown} where a measured value is needed but not
published, and from the standard automotive
literature~\cite{ehsani2018,mitschke2014} otherwise. Where a quantity could
not be traced to any of those, it is an estimate and is marked as one. The
...
12 to \SI{18}{\percent} above \SI{80}{\kilo\metre\per\hour}, and the energy
...
\midrule
 ...
 50 & 212.8 & 207.0 & $-2.7$ \\
 80 & 342.4 & 301.7 & $-11.9$ \\
100 & 458.0 & 389.1 & $-15.0$ \\
130 & 675.2 & 556.6 & $-17.6$ \\
150 & 849.2 & 692.5 & $-18.4$ \\
\bottomrule
\end{tabular}
\end{table}
...
energy, giving \SI{175.3}{\Ah} at a window-averaged open-circuit
voltage of \SI{342.3}{\volt}. A slow discharge then returns
\SI{59.96}{\kWh} at the terminals against a \SI{60.0}{\kWh}
...
\SI{11.15}{\kilo\newton}, so the vehicle is traction-limited up to
\SI{79}{\kilo\metre\per\hour}, which is \SI{77}{\percent} of the time it
takes to reach \SI{100}{\kilo\metre\per\hour}.

Both ways of simplifying the tyre limit fail, and they fail in opposite
directions. Ignoring it altogether leaves only the torque limit and gives
\SI{5.41}{\second}, \SI{12}{\percent} optimistic. Using the static axle load
without the transfer term gives \SI{6.90}{\second}, \SI{12}{\percent}
pessimistic. Only the implicit solution of Equation~\ref{eq:adhesion}, which
lets the load transfer raise the limit as the vehicle accelerates, lands on the
published figure.
...
friction coefficient, cuts below the envelope up to
\SI{79}{\kilo\metre\per\hour}. The dashed curve is the textbook decomposition
...
\SI{0.29}{\percent}. The extra-urban half reproduces its official
\SI{6.955}{\kilo\metre} exactly, so the whole shortfall sits in the urban half.
In the segment list used here those durations sum to \SI{194}{\second} rather
than the specified \SI{195}{\second}, and the missing second is carried in the
final idle; whether that reflects a transcription error or an ambiguity in the
source table was not established.
...
it was replaced by the WLTP~\cite{pavlovic2016,tsiakmakis2017}. Where the WLTC
...
\SI{2.665}{\kWh} net from the battery, giving a consumption of
...
against the net battery energy to \SI{1.3e-7}{\joule}, which is a check on the
book-keeping rather than a coincidence: the drive-unit loss is
...
within its regeneration limit. Figure~\ref{fig:energy} shows the same
decomposition alongside the flow of energy into and out of the battery.
...
Consumption at open circuit     & 117.3 & Wh/km \\
...
speed. The predicted optimum lies near \SI{28}{\kilo\metre\per\hour}.
...
manufacturer publishes. All three are reproduced from a single parameter set,
and none of them was used as a model input.

That claim deserves to be stated precisely, because the distinction between
checking a model and fitting one is easy to blur. Every parameter was set from
an independent source before the comparison was made: published data where it
exists, certification coastdown targets for the road load, and efficiency
targets drawn from the machines literature for the loss model. The published
acceleration, top speed and range were then used to test the result. Where a
deviation appeared during development it was resolved by correcting the physics
rather than by adjusting a parameter until the figure matched. The two
corrections that mattered are described in
Section~\ref{sec:model}: the drive-unit loss model originally made windage
dominant at cruise, and the textbook road-load decomposition omitted the
driveline term entirely. Both were identified from their own internal
implausibility, not from the size of the range error.
...
\SI{213}{\Whpkm} at \SI{130}{\kilo\metre\per\hour}. Neither point entered
any calibration: both follow from the same road-load and drivetrain parameters
that were fixed before the cycle was run. Their magnitude is consistent with
the specific energy consumption reported for battery-electric vehicles in
real-world driving, which for a compact vehicle monitored over four years
varied widely with ambient temperature, traffic and road
grade~\cite{alwreikat2021}. No vehicle-specific measured figure for the
Model~3 at these speeds was available to the authors, so this is offered as a
plausibility check rather than as a validation against a certified value.
\end{loosepar}
...
\addlinespace
Gross capacity \asm{\SI{62.5}{\kWh}} &
...
Coefficients of the form certification prescribes~\cite{epacoastdown},
converted to SI. The values are representative of this vehicle class rather
than read from a certification record for this exact variant, which is why all
three are estimates; the intervals reflect the variation with wheel size, tyre
specification and model year. \\
\addlinespace
...
rear tyres, not the machine, set the acceleration up to
\SI{79}{\kilo\metre\per\hour}, which is \SI{77}{\percent} of the time taken
to reach \SI{100}{\kilo\metre\per\hour}. Dropping the tyre limit entirely is
\SI{12}{\percent} optimistic; keeping it but omitting the dynamic load transfer
is \SI{12}{\percent} pessimistic.
...
using the textbook decomposition alone under-predicts road load by 12 to
\SI{18}{\percent} above \SI{80}{\kilo\metre\per\hour}.
